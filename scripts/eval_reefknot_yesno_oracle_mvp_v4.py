"""MVP v4: spatial-verify with top-edge above/below + idiom blacklist.

Inherits v3 (spatial-verify + asymmetric bias). Two targeted fixes:

  Fix 1 — top-edge for above/below:
    v3 used centre comparison (cy_X < cy_Y) which fails when X sits on top of Y
    with overlapping vertical extent (e.g., "powder topping bread" — powder
    bbox is contained within bread bbox so centres coincide → false 'no').
    v4: use top-edge for 'above' (b_X[1] < b_Y[1]) and bottom-edge for 'below'
    (b_X[3] > b_Y[3]). Catches surface-contact cases.

  Fix 2 — idiom blacklist for 'up'/'down':
    v3 parses "dried up apple" as (dried, up, apple) — a spatial claim.
    "up" / "down" after these verbs are particles, not spatial predicates.
    v4: if word preceding 'up'/'down' is in IDIOM_VERBS_{UP,DOWN}, skip this
    match and try the next relation key.

Usage:
    .venv/bin/python scripts/eval_reefknot_yesno_oracle_mvp_v4.py \\
        --reefknot-jsonl data/reefknot/YESNO.jsonl \\
        --image-dir data/reefknot/images \\
        --n 100 --out outputs/stage0/eval_reefknot_yesno_mvp_v4.json
"""
...
log = logging.getLogger("yesno_mvp_v4")
...
# Verbs whose particle "up" / "down" is idiomatic, not spatial.
# Hits cases like "dried up apple" → not (dried, up, apple).
IDIOM_VERBS_UP = {
    "dried", "drying",
    "stood", "stand", "standing",
    "wake", "wakes", "woken", "waking",
...
IDIOM_VERBS_DOWN = {
    ...
    "put", "puts", "putting",
    "shut", "shuts", "shutting",
    "slow", "slows", "slowing", "slowed",
}


def _is_idiomatic(rel: str, x_part: str) -> bool:
    """True if 'up'/'down' here is a verb particle, not a spatial predicate."""
    if rel not in ("up", "down"):
        return False
    words = x_part.strip().split()
    if not words:
        return False
    prev = words[-1].lower()
    table = IDIOM_VERBS_UP if rel == "up" else IDIOM_VERBS_DOWN
    return prev in table


def _parse_triplet(q: str) -> tuple[str | None, str | None, str | None]:
    """Return (X, REL, Y). Greedy longest-match on REL, with idiom filter."""
    t = _strip_question(q)
    if not t:
        return None, None, None
    for rel in RELATION_KEYS:
        # iterate all occurrences so we can skip idiomatic ones
        for m in re.finditer(rf"(?:^|\s){re.escape(rel)}(?:\s|$)", t):
            x_part = t[:m.start()]
            if _is_idiomatic(rel, x_part):
                continue
            x = x_part.strip()
            y = t[m.end():].strip()
...
def _verify_spatial(predicate: str, box_x: tuple, box_y: tuple,
    ...
    # v4: above/below judged by top-edge AND a "bulk-above" fallback so that
    # surface-contact (X sitting on top of Y, centres coincide) still counts.
    # "above" matches when (a) top of X is above top of Y, OR (b) bottom of X
    # is above the centre of Y (most of X's mass is in Y's upper half).
    if predicate == "y_above":
        top_edge = box_x[1] < box_y[1]
        bulk = box_x[3] < cy_y
        return (top_edge or bulk), f"y1_X={box_x[1]:.0f}<y1_Y={box_y[1]:.0f}={top_edge} bulk={bulk}"
    if predicate == "y_below":
        bot_edge = box_x[3] > box_y[3]
        bulk = box_x[1] > cy_y
        return (bot_edge or bulk), f"y2_X={box_x[3]:.0f}>y2_Y={box_y[3]:.0f}={bot_edge} bulk={bulk}"
    if predicate == "x_left":
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path,
                    default=Path("outputs/stage0/eval_reefknot_yesno_mvp_v4.json"))
    args = ap.parse_args()
