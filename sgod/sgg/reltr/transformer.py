...
Original: https://github.com/yrcong/RelTR/blob/main/models/transformer.py
License: Apache 2.0
"""
import copy
...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, entity_embed, triplet_embed, pos_embed, so_embed):
        ...
        hs, hs_t, sub_maps, obj_maps = self.decoder(
            entity, triplet, memory, memory_key_padding_mask=mask,
            pos=pos_embed, entity_pos=entity_embed,
            triplet_pos=triplet_embed, so_pos=so_embed)

        so_masks = torch.cat((
            sub_maps.reshape(sub_maps.shape[0], bs, sub_maps.shape[2], 1, h, w),
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, entity, triplet, memory,
                ...
                pos=None, entity_pos=None, triplet_pos=None, so_pos=None):
        ...
        for layer in self.layers:
            output_entity, output_triplet, sub_maps, obj_maps = layer(
                output_entity, output_triplet, entity_pos, triplet_pos, so_pos,
        ...
        if self.return_intermediate:
            return (torch.stack(intermediate_entity),
                    torch.stack(intermediate_triplet),
                    torch.stack(intermediate_submaps),
                    torch.stack(intermediate_objmaps))
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt_entity, tgt_triplet, entity_pos, triplet_pos, so_pos,
        ...
        tgt2_entity = self.self_attn_entity(
            q_entity, k_entity, value=tgt_entity, attn_mask=tgt_mask,
            key_padding_mask=tgt_key_padding_mask)[0]
        tgt_entity = tgt_entity + self.dropout2_entity(tgt2_entity)
        tgt_entity = self.norm2_entity(tgt_entity)

        tgt2_entity = self.cross_attn_entity(
            query=self.with_pos_embed(tgt_entity, entity_pos),
        ...
        # triplet layer - coupled self attention
        ...
        tgt2_sub, sub_maps = self.cross_attn_sub(
            query=self.with_pos_embed(tgt_sub, triplet_pos),
            key=self.with_pos_embed(memory, pos), value=memory, attn_mask=memory_mask,
        ...
        tgt2_sub = self.cross_sub_entity(
            query=self.with_pos_embed(tgt_sub, triplet_pos),
        ...
        tgt2_obj, obj_maps = self.cross_attn_obj(
            query=self.with_pos_embed(tgt_obj, triplet_pos),
            key=self.with_pos_embed(memory, pos), value=memory, attn_mask=memory_mask,
            key_padding_mask=memory_key_padding_mask)
        ...
        tgt2_obj = self.cross_obj_entity(
            query=self.with_pos_embed(tgt_obj, triplet_pos),
            key=tgt_entity, value=tgt_entity)[0]
